...
import { useRackCreate, useRacksList, useRackRemove, useRackUpdate } from '../hooks/useRacks';
...
export const RacksManagement: React.FC = () => {
  ...
  const [editForm, setEditForm] = useState<{code?: string; name?: string; location?: string}>({});

  const { data, refetch } = useRacksList();
  useEffect(() => { setItems((data?.data as RackModel[]) ?? []); }, [data]);

  const create = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    try { await racksService.create(form); await refetch(); setForm({ code: '' }); } finally { setLoading(false); }
  };
  const remove = async (id: string) => { await racksService.remove(id)(); await refetch(); };
  const startEdit = (r: RackModel) => { setEditingId(r.id); setEditForm({ code: r.code, name: r.name ?? undefined, location: r.location ?? undefined }); };
  const cancelEdit = () => { setEditingId(null); setEditForm({}); };
  const saveEdit = async (id: string) => {
    if (!editForm.code || !editForm.code.trim()) return;
    await racksService.update(id)({ code: editForm.code, name: editForm.name, location: editForm.location });
    setEditingId(null);
    setEditForm({});
    await refetch();
  };
            ...
            {editingId === r.id ? (
              <>
                <div className="flex items-center justify-between">
                  <input className="border p-2 rounded w-40" value={editForm.code || ''} onChange={e=>setEditForm({...editForm, code: e.target.value})} />
                  <Badge variant={r.status === 'AVAILABLE' ? 'success' : r.status === 'OCCUPIED' ? 'warning' : 'secondary'} size="sm">{r.status}</Badge>
                </div>
                <input className="border p-2 rounded" placeholder="Nama" value={editForm.name || ''} onChange={e=>setEditForm({...editForm, name: e.target.value})} />
                <input className="border p-2 rounded" placeholder="Lokasi" value={editForm.location || ''} onChange={e=>setEditForm({...editForm, location: e.target.value})} />
                <div className="flex gap-3 justify-end">
                  <button onClick={()=>cancelEdit()} className="text-slate-500 hover:underline text-sm">Batal</button>
                  <button onClick={()=>saveEdit(r.id)} className="text-blue-600 hover:underline text-sm">Simpan</button>
                </div>
              </>
            ) : (
              <>
                ...
                <div className="flex gap-3 justify-end">
                  <button onClick={()=>startEdit(r)} className="text-blue-600 hover:underline text-sm">Ubah</button>
