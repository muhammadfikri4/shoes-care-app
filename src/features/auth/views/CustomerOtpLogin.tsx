    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white flex items-center justify-center p-4">
      <div className="w-full max-w-xl grid md:grid-cols-2 grid-cols-1 gap-6">
        <div className="hidden md:flex flex-col justify-center rounded-2xl p-8 bg-[conic-gradient(at_top_left,_var(--tw-gradient-stops))] from-emerald-500 via-teal-500 to-cyan-500 text-white shadow-xl">
          ...
          <p className="text-white/90 mt-2 text-sm">Gunakan email untuk menerima OTP. Simpel dan aman.</p>
        ...
        <div className="bg-white rounded-2xl p-8 border border-slate-100 shadow-xl">
          <h1 className="text-xl font-semibold mb-4">{sent ? 'Masukkan Kode OTP' : 'Dapatkan Kode OTP'}</h1>
          {!sent ? (
            <form onSubmit={requestOtp} className="space-y-4">
              ...
              <button disabled={loading} className="w-full bg-emerald-600 hover:bg-emerald-700 transition text-white rounded py-2">{loading ? 'Mengirim...' : 'Kirim OTP'}</button>
            </form>
          ) : (
            <form onSubmit={verifyOtp} className="space-y-4">
              <div>
                <label className="text-sm">Kode OTP</label>
                <input className="w-full border rounded p-2 tracking-widest" value={otp} onChange={e=>setOtp(e.target.value)} placeholder="6 digit" required />
              </div>
              <button disabled={loading} className="w-full bg-blue-600 hover:bg-blue-700 transition text-white rounded py-2">{loading ? 'Memverifikasi...' : 'Verifikasi & Masuk'}</button>
              <button type="button" onClick={()=>setSent(false)} className="w-full text-blue-600 underline text-sm">Kirim ulang ke email lain</button>
            </form>
          )}
          {message && <p className="text-sm text-gray-600 mt-4">{message}</p>}
          <div className="mt-6 text-sm text-slate-600">Admin? <a href="/login" className="text-blue-600 hover:underline">Masuk di sini</a></div>
        </div>
      </div>
    </div>
  );
